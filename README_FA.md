...
| خطای `Text-preservation check failed` برای یک پاراگراف | نسخه 2.0.5 یا جدیدتر را نصب کنید. نسخه‌های قبلی اگر پیش از فرمول لینک، Smart Tag یا Track Changes وجود داشت، ممکن بود محل فرمول را اشتباه محاسبه کنند؛ فایل اصلی در تمام حالت‌ها محافظت شده و تغییر نکرده است |
| حالت غیراتمیک پس از رد یک فرمول پیچیده باز هم متوقف می‌شود | نسخه 2.0.6 یا جدیدتر را نصب کنید؛ فرمول ردشده دست‌نخورده می‌ماند، فرمول‌های امن تبدیل می‌شوند و خروجی معتبر همراه هشدار ساخته می‌شود |
| تعداد زیادی فرمول معتبر به‌عنوان Run پیچیدهٔ Word رد می‌شوند | نسخهٔ 2.0.7 یا جدیدتر را نصب کنید؛ نشانگرهای خودکار غلط‌یابی و صفحه‌بندی Word دیگر مانع تبدیل امن به فرمول بومی نمی‌شوند |
| Google Docs خروجی را باز می‌کند اما Microsoft Word خطا می‌دهد | نسخهٔ 2.0.8 یا جدیدتر را نصب کنید. نسخه‌های قبلی ممکن بود فرمول نمایشی Office Math را بیرون از پاراگراف الزامی Word قرار دهند؛ نسخهٔ 2.0.8 تمام OMML را داخل `w:p` نگه می‌دارد و خروجی ناسازگار را اصلاً منتشر نمی‌کند |
...
- **اصلاح سازگاری Microsoft Word در 2.0.8:** فرمول نمایشی OMML داخل `w:p` می‌ماند و کنترل سخت‌گیرانه پیش از انتشار اجرا می‌شود.
- **اصلاح نشانگرهای Word در 2.0.7:** نشانگرهای خودکار غلط‌یابی و صفحه‌بندی دیگر باعث رد فرمول‌های معتبرِ تقسیم‌شده میان Runهای عادی نمی‌شوند.
- **اصلاح حالت غیراتمیک در 2.0.6:** فرمول پیچیده ردشده حفظ می‌شود، تبدیل فرمول‌های امن ادامه می‌یابد و خروجی معتبر ساخته می‌شود.
- **اصلاح حفظ متن Word در 2.0.5:** محاسبه هماهنگ موقعیت کنار لینک‌ها و Track Changes و توضیح فارسی توقف ایمن.
